##Rapid test sensitivity analysis
##12/18/2021
##Analyst: Emily Stein
...
#Only keeping important variables from cheif complaint dataset
ChiefComplaint_Short <- ChiefComplaint %>%
  select(VisitID, chills:confusion, Vax_rec, Vax_date, Fully_vax)%>%
  distinct()

####Vital Sign Cleaning####
#Merge Vital stats data
VitalSigns <- read_delim("D:/12-15-2021/CUNY_VitalSigns.csv", delim="|",escape_double = FALSE, trim_ws = TRUE)
#restrict dates 
...
VitalSigns %>%
  mutate(Fever.vitalsigns = ifelse(Temp>=100.4, 1,0))-> VitalSigns

#Create a variable for O2 saturation
VitalSigns %>%
  mutate(O2.vitalsigns = ifelse(O2sat<=95, 1,0))-> VitalSigns

####Merging chief complaint and vital sign data####
ChiefComplaint_Short<-ChiefComplaint_Short%>%
  distinct()

VitalSigns<-VitalSigns%>%
  distinct()

merged_chief.vital<-inner_join(ChiefComplaint_Short, VitalSigns, by="VisitID")
###Creating symptomatic variable based on Madhura's coding from Chief Complaint symptoms and Vital Signs####
merged_chief.vital %>%
  mutate(fever.final = ifelse(fever=="Yes" | Fever.vitalsigns==1, "Yes", "No"),
         symptomatic.case = ifelse((fever.final=="Yes" | chills=="Yes" | cough =="Yes" | sore.throat=="Yes" | fatigue=="Yes" | headache=="Yes"|
                                      ...
                                      congestion.nose=="Yes" | body.ache=="Yes" | O2.vitalsigns==1), 1, 0))%>%
  distinct()-> merged_chief.vital 


#Selecting variables of interest
chief.vital<-merged_chief.vital%>%
  select(VisitID, Vax_rec, Vax_date, Fully_vax, symptomatic.case)%>%
  distinct()

#Filling in symptomatic values for duplicate VisitIDs that are missing it on one line
chief.vital.new<-chief.vital%>%
  group_by(VisitID)%>%
  arrange(desc(symptomatic.case))%>%
  fill(symptomatic.case, .direction = c("down"))%>%
  ungroup()%>%
  distinct()

#Checking to make sure there are no duplicates
chief.vital.new[duplicated(chief.vital.new$VisitID),]

#Saving COVID Results file
saveRDS(merged_chief.vital, file = "ChiefComplaint_Vax_Symp_FULL.RDS")
saveRDS(chief.vital.new, file = "ChiefComplaint_Vax_Symp_SHORT.RDS")
...
updatedrace_orig_race_Oct<-read_csv("D:/12-15-2021/Updated race data/updatedrace_orig_race_Oct.csv")

updatedrace_orig_race_Oct <- updatedrace_orig_race_Oct %>%
  group_by(PatientID) %>%
  mutate(Visit = 1:n())

#making data wide
updatedrace_orig_race_Oct<- as.data.table(updatedrace_orig_race_Oct)

updatedrace_orig_race_Oct.w.re <- dcast(updatedrace_orig_race_Oct, PatientID  ~ Visit, value.var = "Race")
updatedrace_orig_race_Oct.race <- updatedrace_orig_race_Oct.w.re[,1:2]

updatedrace_orig_race_Oct.w.ethnicity <-  dcast(updatedrace_orig_race_Oct, PatientID  ~ Visit, value.var = "Ethnicity")
updatedrace_orig_race_Oct.ethnicity <- updatedrace_orig_race_Oct.w.ethnicity[,1:2]
updatedrace_orig_race_Oct.w <- left_join(updatedrace_orig_race_Oct.race, updatedrace_orig_race_Oct.ethnicity, by="PatientID")

updatedrace_orig_race_Oct<-updatedrace_orig_race_Oct.w%>%
  rename(Race=`1.x`,
         Ethnicity=`1.y`)
...
updated.race%>%
  distinct()->updated.race

Visit.demo.new <- left_join(Visit.demo, updated.race, by = "PatientID")

#update racecat var
Visit.demo.new$racecat <- if_else(Visit.demo.new$racecat=="Other/Unknown" & !(is.na(Visit.demo.new$new_racecat)), Visit.demo.new$new_racecat, Visit.demo.new$racecat)

Visit.demo.new<-Visit.demo.new%>%
  distinct()
...
saveRDS(Visit.demo.new, file = "Visit_demo_2.25.22.RDS")
...
###MERGING DATA####
#If loading previously saved datasets
COVIDResults_confPCR<-readRDS("COVIDResults_confPCR.RDS")
Visit.demo.new<-readRDS("Visit_demo_2.25.22.RDS")
chief.vital.new<-readRDS("ChiefComplaint_Vax_Symp_SHORT.RDS")
#Merge Visit data with covid results to get patient IDs for all results 
merged.data1 <- inner_join(COVIDResults_confPCR, Visit.demo.new, by="PatientID") #innerjoin because out of state covid results need to be filtered out (6994684)
...
merged.data2 <- left_join(merged.data1, chief.vital.new, by="VisitID") #9226028
...
write_rds(main, "main_2.24.22.RDS")

vax<-main%>%
  filter(Test_date >= "2021-04-01" & !is.na(vax_status))

write_rds(main, "vax_2.24.22.RDS")
####Load cleaned final dataset####
#Whole dataset
main<-readRDS("main_2.24.22.RDS")
#Vaccinated dataset
...
#need to add race
main<-main%>%
  mutate(symptomatic.case.character=ifelse(is.na(symptomatic.case), "no", "yes"))

table1<-main%>%
  select(Age, agegrp_20, racecat, Gender, symptomatic.case.character, vax_status, antigen_result)
...
#Overall ORF CT boxplot
overall_ct_orf<-ggplot(orf, aes(x=Performance, y=ct_result)) + 
  #scale_color_manual(values=c('Blue','Red'))+
  geom_boxplot() + 
  xlab("")+
  ylab("PCR CT")+
...
overall_ct_orf<-overall_ct_orf+ 
  geom_signif(comparisons = list(c("PCR+/RT-", "PCR+/RT+")))

#Overall Egene CT boxplot
overall_ct_egene<-ggplot(egene, aes(x=Performance, y=ct_result)) + 
  #scale_color_manual(values=c('Blue','Red'))+
  geom_boxplot() + 
  xlab("")+
  ylab("PCR CT")+
...
overall_ct_egene<-overall_ct_egene+ 
  geom_signif(comparisons = list(c("PCR+/RT-", "PCR+/RT+")))


#Combining overall boxplots for supp table 2

lay <- rbind(c(1,1,2, 2),
             c(1,1,2, 2))

gridExtra::grid.arrange(overall_ct_orf,overall_ct_egene, layout_matrix=lay)

#Vaccinated CT boxplot
orf_fullvax<-orf%>%
  filter(vax_status=="Fully vaccinated")

full_vax_ct<-ggplot(orf_fullvax, aes(x=Performance, y=ct_result)) + 
  #scale_color_manual(values=c('Blue','Red'))+
  geom_boxplot() + 
  xlab("")+
  ylab("PCR CT")+
...
full_vax_ct<-full_vax_ct+ 
  geom_signif(comparisons = list(c("PCR+/RT-", "PCR+/RT+")))
#Partially vaccinated CT boxplot
orf_partvax<-orf%>%
  filter(vax_status=="Partially vaccinated")

part_vax_ct<-ggplot(orf_partvax, aes(x=Performance, y=ct_result)) + 
  #scale_color_manual(values=c('Blue','Red'))+
  geom_boxplot() + 
  xlab("")+
  ylab("PCR CT")+
...
un_vax_ct<-ggplot(orf_unvax, aes(x=Performance, y=ct_result)) + 
  #scale_color_manual(values=c('Blue','Red'))+
  geom_boxplot() + 
  xlab("")+
  ylab("PCR CT")+
...
gridExtra::grid.arrange(full_vax_ct, part_vax_ct, un_vax_ct, layout_matrix = lay)

#Supplementary figure 3: Vax status for egene ct values
#Vaccinated CT boxplot
egene_fullvax<-egene%>%
  filter(vax_status=="Fully vaccinated")

full_vax_ct<-ggplot(egene_fullvax, aes(x=Performance, y=ct_result)) + 
  #scale_color_manual(values=c('Blue','Red'))+
  geom_boxplot() + 
  xlab("")+
  ylab("PCR CT")+
...
part_vax_ct<-ggplot(egene_partvax, aes(x=Performance, y=ct_result)) + 
  #scale_color_manual(values=c('Blue','Red'))+
  geom_boxplot() + 
  xlab("")+
  ylab("PCR CT")+
...
un_vax_ct<-ggplot(egene_unvax, aes(x=Performance, y=ct_result)) + 
  #scale_color_manual(values=c('Blue','Red'))+
  geom_boxplot() + 
  xlab("")+
  ylab("PCR CT")+
