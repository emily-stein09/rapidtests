...
vars=c("chills", "cough", "fever", "fatigue", "body.ache", "headache", "taste.smell",
       "sore.throat", "congestion.nose", "nausea.vomit", "diarrhea", "sob", "chest.pain",
       "confusion", "Fully_vax", "Vax_rec")

ChiefComplaint_symp_vax<-ChiefComplaint%>%
  mutate_at(vars(all_of(vars)), funs(old = na_if(., ''))) %>% 
  # set grouping for following operations
  group_by(VisitID) %>% 
  # for added columns, fill values downwards and upwards within each group
  fill(vars) %>% 
  fill(vars, .direction = 'up') %>%
  # reinsert empty strings for NAs
  mutate_at(vars(vars), funs(coalesce(., factor(NA))))%>%
  ungroup()

vars=c("chills", "cough", "fever", "fatigue", "body.ache", "headache", "taste.smell",
       "sore.throat", "congestion.nose", "nausea.vomit", "diarrhea", "sob", "chest.pain",
       "confusion", "Fully_vax", "Vax_rec", "VisitID")

ChiefComplaint_symp_vax<-ChiefComplaint_symp_vax[vars]%>%
  distinct()
...
ChiefComplaint<-readRDS("ChiefComplaint_Vax_Symp_Clean.RDS")

merged.data1 <- inner_join(COVIDResults, Visit, by="PatientID") #innerjoin because out of state covid results need to be filtered out (6994684)
